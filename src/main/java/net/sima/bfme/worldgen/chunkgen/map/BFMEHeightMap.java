...
import net.sima.bfme.worldgen.map.BFMEMapUtils;
import java.awt.*;
import java.util.ArrayList;

public class BFMEHeightMap {
    ...
    public static final float MOUNTAIN_HEIGHT_MULTIPLIER = 1.97f;
    public static final float MOUNTAIN_EXPONENTIAL_HEIGHT = 1.207f;
    ...
    private static final int PIXEL_WEIGHT = BFMEMapConfigs.PIXEL_WEIGHT;
    public static final ArrayList<Float> percentages = new ArrayList<Float>();
    private static BFMEMapRuntime BFMEMapRuntime;
    private static Float defaultWeightHeight = null;

    public BFMEHeightMap(){
        BFMEMapRuntime = BFMEMapRuntime.getInstance();
    }

    private static float getImageHeight(int xWorld, int zWorld) {
        if(BFMEMapRuntime == null) BFMEMapRuntime = BFMEMapRuntime.getInstance();

        Color color = BFMEMapRuntime.getHeight(xWorld, zWorld);
        ...
        return BFMEBiomesData.defaultBiome.height * WATER_MULTIPLIER;
    }
    ...
    private static float getPerlinMapHeight(int x, int z) {
        ...
        if(BFMEMapUtils.getInstance().isWorldCoordinateInBorder(x,z)) {
    ...
    private static float getSmoothHeight(int x, int z) {
        ...
        for(int i = -SMOOTH_BRUSH_SIZE; i <= SMOOTH_BRUSH_SIZE; i++) {
            for(int j = -SMOOTH_BRUSH_SIZE; j <= SMOOTH_BRUSH_SIZE; j++) {
                if(BFMEMapUtils.getInstance().isWorldCoordinateInBorder(x + i, z + j)) total += BFMEBiomesData.defaultBiome.height;
