 ...
 *
 * Лучший выбор для террейна:
 * - Нет диагональных артефактов
 * - Плавные переходы (RSQUARED = 2/3)
 * - Без патентных ограничений
 */
public class OpenSimplex2S {
     ...
     * ИСПОЛЬЗУЙ ЭТОТ МЕТОД ДЛЯ HEIGHTMAP!
     */
     ...
     * НЕ использовать для heightmap — только для 2D игр с вертикальной Y.
     */
     ...
     * ИСПОЛЬЗУЙ ЭТОТ МЕТОД ДЛЯ ПЕЩЕР (X, Z — горизонталь, Y — вертикаль)
     */
     ...
     * Альтернатива для пещер если Z — вертикаль.
     */
    ...
    /**
     * 3D OpenSimplex2S/SuperSimplex noise base.
     */
        ...
        if (a2 > 0) {
            ...
            if (a4 > 0) {
                ...
                value += (a4 * a4) * (a4 * a4) * grad(seed2,
                        xrbp + (xNMask & (PRIME_X << 1)), yrbp + PRIME_Y, zrbp + PRIME_Z, x4, y1, z1);
                skip5 = true;
            }
        }
        ...
        if (!skip9) {
            ...
            if (a9 > 0) {
                ...
                value += (a9 * a9) * (a9 * a9) * grad(seed2,
                        xrbp + (xNMask & (PRIME_X << 1)), yrbp + PRIME_Y, zrbp + (zNMask & (PRIME_Z << 1)), x9, y1, z9);
            }
        }
